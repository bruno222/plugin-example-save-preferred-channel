...
import { ITask, Actions, withTaskContext, templates, Template, useFlexSelector } from '@twilio/flex-ui';
...
const TaskWrapUpForm = ({ task }: ComponentProps) => {
  ...
  const saveForm = async () => {
    ...
    await fetch('https://manual-tests-9183.twil.io/set-customer-preferences', {
      method: 'POST',
      mode: 'cors',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ ...convData, ...task.attributes }),
    });
      ...
      <Table variant="borderless">
        <TBody>
          <Tr key="reason">
            ...
            <Td>
              <Input id="reason" type="text" value={reason} onChange={handleChange} />
            </Td>
          </Tr>
          <Tr key="topic">
            <Th scope="row">
              <Label htmlFor="topic">
                <Template source={templates.PreferredChannel} />
              </Label>
            </Th>
            <Td>
              <Select value={topic} onChange={handleTopicChange} id="topic">
                {topics.map((topic) => {
                  if (topic.disabled) {
                    return (
                      <Option disabled key={topic.value} value={topic.value}>
                        {' '}
          ...
          <Tr key="outcome">
            ...
            <Td>
              <Select value={outcome} onChange={handleOutcomeChange} id="outcome">
                {outcomes.map((option) => {
                  if (option.disabled) {
                    return (
          ...
          <Tr key="newCustomer">
            <Td />
            <Td>
              <Checkbox id="newCustomer" value="customerStatus" name="customerStatus" checked={isNewCustomer} onChange={onNewCustomerChange}>
          ...
          <Tr key="fraudAlert">
            <Td />
            <Td>
              <Checkbox id="fraud" value="fraud" name="customerStatus" checked={fraudAlert} onChange={onFraudChange}>
          ...
          <Tr key="button">
            <Td>[Lang: {language}]</Td>
            <Td>
              <Button variant="primary" size="small" id="saveButton" onClick={saveForm} disabled={!changed}>
